/* Phase 6 chain sanity — the THREE real ports (anamorphic_lab → bokeh →
 * analog) composed in one engine, real MediaPipe path, playing video:
 * chain order + enabled flags, GL errors, non-black output, fps reported
 * honestly (GPU criterion). Fresh-session standalone script (see the
 * phase 5 chain script for why this lives outside the behavior suite). */
...
(async () => {
  ...
  for (const id of ['anamorphic_lab', 'bokeh', 'analog']) {
  ...
  await page.waitForFunction(() => {
    ...
    return !!c && c.length >= 3 && c[0]?.id === 'anamorphic_lab' && c[0]?.enabled && c[1]?.id === 'bokeh' && c[1]?.enabled && c[2]?.id === 'analog' && c[2]?.enabled;
  }, null, { timeout: 300000 });
  ...
  step('PersonMask READY on the real path (fresh session, 3-node chain)', segReady, '');
  ...
  const restOff = stat.chain.split('→').slice(3).every((s) => s.endsWith('(off)'));
  step('anamorphic_lab→bokeh→analog chain renders (no GL errors, non-black, rest bypassed)',
    stat.glErr === 0 && stat.meanLum > 2 && /^anamorphic_lab→bokeh→analog(→|$)/.test(stat.chain) && restOff,
    JSON.stringify(stat));
  step('no page errors', pageErrors.length === 0, pageErrors.slice(0, 4).join(' | '));
  fs.writeFileSync(path.join(SCRATCH, 'phase6-chain-summary.json'), JSON.stringify(stat, null, 2));
